...
Recently several [generative ui specs](./concepts/generative-ui-specs) (including MCP-UI, Open JSON UI, and A2UI) have been released which allow agents to deliver UI widgets through the interaction protocols.  AG-UI works with all of these.  Visit our [generative ui specs page](./concepts/generative-ui-specs) to learn more.
...
# Interrupts
Source: https://docs.ag-ui.com/concepts/interrupts
...
Every message type carries `metadata`, an optional open-by-key object that
accumulates as the message is built from its events. Tool calls carry one of
their own. See [Metadata](/concepts/metadata) for the rules.
...
# Metadata
Source: https://docs.ag-ui.com/concepts/metadata
...
Events and messages carry an optional [`metadata`](/concepts/metadata) object,
and it is part of what you are serializing — persist it alongside everything
else or a replayed stream will be missing token usage, trace ids and anything
else producers attached.

Two things to know when persisting:

* **Compaction folds it.** Collapsing deltas merges their metadata, last write
  wins. See [Event Compaction](#event-compaction) and the
  [compaction reference](/sdk/js/client/compaction) for the ordering caveats.
* **Binary is narrower than JSON.** Over protobuf, metadata is a
  `google.protobuf.Struct`, so nulls, arrays and nested objects survive — but
  numbers are IEEE-754 doubles, so integers beyond 2^53 lose precision, and
  several event types have no protobuf representation at all. JSON round-trips
  everything exactly. Prefer JSON for archival storage.
...
* Concepts: [Events](/concepts/events), [Metadata](/concepts/metadata),
...
All three of these events can carry a `metadata` object, and it accumulates onto
the **tool call itself** rather than onto the assistant message that owns it:
...
That is deliberate. One assistant message can own several tool calls, so folding
their metadata into the parent would make the result depend on the order the
calls happened to interleave. Giving each tool call its own keeps it
unambiguous.

`ToolCallResult` is different — it creates a tool message, so its metadata
merges into that message like any other message-building event. See
...
<CardGroup>
  ...
  <Card title="Interrupt-Aware Run Lifecycle" href="/concepts/interrupts" icon="file-lines">
...
# Events
Source: https://docs.ag-ui.com/sdk/dotnet/abstractions/events
...
| `InputTokens`       | `inputTokens`       | `long?`   | Input tokens, including cached input tokens |
| `OutputTokens`      | `outputTokens`      | `long?`   | Output tokens, including reasoning tokens   |
| `TotalTokens`       | `totalTokens`       | `long?`   | Total tokens billed                         |
| `ReasoningTokens`   | `reasoningTokens`   | `long?`   | Output tokens spent on internal reasoning   |
| `CachedInputTokens` | `cachedInputTokens` | `long?`   | Input tokens served from cache              |
...
# Multimodal Inputs
Source: https://docs.ag-ui.com/sdk/dotnet/abstractions/multimodal-inputs
...
# Overview
Source: https://docs.ag-ui.com/sdk/dotnet/abstractions/overview
...
<CardGroup>
  <Card title="Types Reference" icon="cube" href="/sdk/dotnet/abstractions/types">
    Complete documentation of the protocol types in `AGUI.Abstractions`
  </Card>

  <Card title="Multimodal Inputs" icon="image" href="/sdk/dotnet/abstractions/multimodal-inputs">
...
<Card title="Events Reference" icon="bolt" href="/sdk/dotnet/abstractions/events">
  Complete documentation of all event types in `AGUI.Abstractions`
</Card>


# Types
Source: https://docs.ag-ui.com/sdk/dotnet/abstractions/types
...
# AGUIChatClient
Source: https://docs.ag-ui.com/sdk/dotnet/client/chat-client
...
# Overview
Source: https://docs.ag-ui.com/sdk/dotnet/client/overview
...
<Card title="AGUIChatClient Reference" icon="comments" href="/sdk/dotnet/client/chat-client">
...
<Card title="Transport Reference" icon="network-wired" href="/sdk/dotnet/client/transport">
...
# Transport
Source: https://docs.ag-ui.com/sdk/dotnet/client/transport
...
# Endpoints
Source: https://docs.ag-ui.com/sdk/dotnet/hosting/endpoints
...
# Extensibility
Source: https://docs.ag-ui.com/sdk/dotnet/hosting/extensibility
...
# Overview
Source: https://docs.ag-ui.com/sdk/dotnet/hosting/overview
...
<CardGroup>
  <Card title="Endpoints" icon="server" href="/sdk/dotnet/hosting/endpoints">
    Build the canonical ASP.NET Core AG-UI endpoint
  </Card>

  <Card title="Extensibility" icon="layer-group" href="/sdk/dotnet/hosting/extensibility">
...
prompts, completions, messages, or identifiers.
...
| `inputTokens`       | `number` (optional) | Prompt/input tokens                              |
| `outputTokens`      | `number` (optional) | Completion/output tokens                         |
| `totalTokens`       | `number` (optional) | Total tokens for the entry                       |
| `reasoningTokens`   | `number` (optional) | Reasoning tokens, when the model reports them    |
| `cachedInputTokens` | `number` (optional) | Cached (prompt-cache read) input tokens          |
...
# Multimodal Inputs
Source: https://docs.ag-ui.com/sdk/js/core/multimodal-inputs
...
| `metadata`  | `Optional[Dict[str, Any]]` | Extra information attached to the event               |

`metadata` is open by key: any JSON value is allowed under a key, including
`None`. The object may be absent, but a present one is never `None` — an
explicit `null` is read back as absent, so a plain `model_dump_json()`
round-trips. `EventEncoder` uses `exclude_none=True`, so an absent object is
omitted from the wire rather than emitted as `null`. The `ag-ui` key is reserved
for AG-UI's own use.
...
| Property              | Type            | Description                                      |
| --------------------- | --------------- | ------------------------------------------------ |
| `provider`            | `Optional[str]` | Provider that reported the usage (e.g. `openai`) |
| `model`               | `Optional[str]` | Model that produced the usage                    |
| `input_tokens`        | `Optional[int]` | Prompt/input tokens                              |
| `output_tokens`       | `Optional[int]` | Completion/output tokens                         |
| `total_tokens`        | `Optional[int]` | Total tokens for the entry                       |
| `reasoning_tokens`    | `Optional[int]` | Reasoning tokens, when the model reports them    |
| `cached_input_tokens` | `Optional[int]` | Cached (prompt-cache read) input tokens          |
...
| `usage`     | `Optional[List[TokenUsage]]` | Per-(provider, model) token usage for the run, when reported |
...
| `usage`   | `Optional[List[TokenUsage]]` | Partial token usage from model calls that completed |
...
# Multimodal Inputs
Source: https://docs.ag-ui.com/sdk/python/core/multimodal-inputs
...
# Support for AG-UI
Source: https://docs.ag-ui.com/support

Open-source and community support for people building with AG-UI
...
<CardGroup>
  <Card title="Bugs, feature requests, and contributions" icon="code-branch" href="https://github.com/ag-ui-protocol/ag-ui/issues">
    Report a problem, propose an improvement, or contribute to AG-UI on GitHub.
  </Card>

  <Card title="Community questions and discussion" icon="comments" href="https://discord.gg/Jd3FzfdJa8">
...
# Production support
Source: https://docs.ag-ui.com/talk-to-us

Optional commercial architecture and implementation support for teams deploying AG-UI
...
<div>
  <a href="https://www.copilotkit.ai/talk-to-an-engineer?ref=ag_ui_docs_production_support&utm_source=docs&utm_medium=referral&utm_campaign=production_support&utm_content=ag_ui_production_support">
